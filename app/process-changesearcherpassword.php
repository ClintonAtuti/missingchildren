...
if (isset($_POST['btnChangePass'])) {
	...
	$sql="SELECT `Password` FROM familyMember WHERE `Username`='".$_SESSION['familyMember']."'";
	...
	if (strlen($newpass)<8) {
		...
		<script type="text/javascript">
			...
			window.location.assign('../views/familyMember/editadminprofile.php');
		</script>
		<?php
		...
		$sqlUpdate="UPDATE `missingchild`.`familyMember` SET `Password` = '".$encyptedpass."' WHERE `familyMember`.`Username` = '".$_SESSION['familyMember']."' ";
		//run query
