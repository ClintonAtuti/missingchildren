...
if(isset($_POST['submit'])){
	...
	$sql_verifyid="SELECT * FROM `registry` WHERE `National_ID`='".$national_id."'";
	$sql_run=mysqli_query($conn,$sql_verifyid);
	$sql_num=mysqli_num_rows($sql_run);
	if($sql_num!=1){
		header("refresh:3;url=../views/familyMember/reportmissingchild.php");
	...
	$sql="INSERT INTO `missingchild`.`missing` (`ID`, `Full_Name`, `Image`, `Age`, `Height`, `Weight`, `Gender`, `SkinColor`, `HairColor`, `EyeColor`, `Date_Missing`, `Description`, `Date_Reported`,`Reporter_ID`) VALUES (NULL, '".$fullname."', '".$image."', '".$age."', '".$height."', '".$weight."', '".$gender."', '".$skincolor."', '".$haircolor."', '".$eyecolor."', '".$datemissing."', '".$desc."', CURRENT_TIMESTAMP,'".$national_id."')";
   ...
   window.location="../views/familyMember/familyMemberdashboard.php"
    </script>
    <?php
		}else{
