        ...
        if($num_rows==1){
			...
			if($num_rows_user2==1){
				...
				if($role=="Authorised_Party"){
					...
					$sql3="SELECT `Username`,`Password`,`Phone_No`,`Email`,`First_Name`,`Middle_Name`,`Last_Name`,`Status` FROM `familyMember` WHERE  Binary `Username`=Binary '".$userid."' AND Binary `Password`=Binary '".$password."' ";
				...
				$_SESSION['familyMember']=$userid3;
					
					if($status=="1"){
							header('Location: ../views/familyMember/familyMemberdashboard.php');
						}//end of if status is active
						else if($status=="0"){
							echo "Dear ".$_SESSION['familyMember']." Your account is blocked, contact system administrator for more details";//redirect to active familyMember dashboard
							}//end of if familyMember is deactivated
				...
				echo '<p><h1>Invalid Credentials</h1></p>';
					...
					}//end of if a familyMember is found
