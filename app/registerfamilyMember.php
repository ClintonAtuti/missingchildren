...
require_once('../config/security.php');
		...
		if(isset($_POST['register-submit'])){
			...
			$sql="SELECT `National_ID` FROM `registry` WHERE `National_ID`='".$nationalID."'";
			...
			if($query_run==1){
				...
				if($password==$confirmpass){
					...
					if(strlen($password)>=6){
						//prepare insert
						$_SESSION['familyMembername']=$_POST['username'];
						
						
						$encyptpass=md5($password);
						$sqlAddfamilyMember="INSERT INTO `missingchild`.`familyMember` (`Username`, `Password`,`ID_No`,`Email`) VALUES (?,?,?,?)";
						
						//prepare sql
						$sql_prepare=mysqli_prepare($conn,$sqlAddfamilyMember);
