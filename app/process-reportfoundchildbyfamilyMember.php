...
//check if the button report a found child isclicked
...
if (isset($_POST['submit'])) {
	...
	$sql3="SELECT `National_ID` FROM registry WHERE `National_ID`='".$idno."'";
	$query_run3=mysqli_query($conn,$sql3);
	$sql_num_rows3=mysqli_num_rows($query_run3);
	if($sql_num_rows3==1){

        //obtain email address of the missing child reporter from admin table.
        ...
        elseif ($sql_num_rowemail==0) {
            ...
            elseif ($sqlnumusermail==0) {
                //check email in familyMember table
                $sqlfamilyMembermail="SELECT `Email` FROM familyMember WHERE `ID_No`='".$_SESSION['reporter_id']."' ";
                $sqlrunfamilyMembermail=mysqli_query($conn,$sqlfamilyMembermail);
                $sqlnumfamilyMembermail=mysqli_num_rows($sqlrunfamilyMembermail);
                if ($sqlnumfamilyMembermail==1) {
                    while ($rowfamilyMembermail=mysqli_fetch_assoc($sqlrunfamilyMembermail)) {
                        $reporteremail=$rowfamilyMembermail['Email'];
                    }
                }//end of if
            }
        }
   ...
   window.location="../views/familyMember/emailconcernedchild.php"
    </script>
    <?php
		...
		header("Refresh: 5; url=../views/familyMember/displaymissingchild.php");
